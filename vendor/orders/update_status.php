...
<div class="container-fluid">
    <form action="" id="update_status">
        ...
        <div class="form-group">
			...
			<?php 
			$user = $conn->query("SELECT quantity FROM product_list where id ='".$_settings->userdata('id')."'");
				foreach($user->fetch_array() as $l =>$m){
					$$l = $m;
				}
            ...
            <select name="status" id="status" class="form-control rounded-0" required>
                ...
                <option <?= isset($status) && $status == 4 ? 'selected' : '' ?> value="4">Delivered
			<?php
			require_once('./../../config.php');
			if(isset($_GET['id'])){
			if($status == 4){
				$qry = $conn->query("UPDATE `product_list` SET `quantity` = quantity-1 WHERE `product_list`.`id` = 11");
				}
			}
			?></option>
